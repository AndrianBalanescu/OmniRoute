...
test("(#3696) non-tiered ANTIGRAVITY_PUBLIC_MODELS entries resolve to distinct upstream ids", () => {
  ...
  for (const model of ANTIGRAVITY_PUBLIC_MODELS) {
    // Gemini 3.7/3.8 Flash tiers intentionally share the upstream `...-flash-tiered`
    // endpoint with different reasoning token budgets, and the bare `-flash` id is a
    // display alias of that tiered model (see ANTIGRAVITY_MODEL_ALIASES). Both families
    // are therefore exempt from the distinct-upstream-id invariant below.
