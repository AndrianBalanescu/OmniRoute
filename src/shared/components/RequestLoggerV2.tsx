...
// Persisted "hide model/provider" sets so the user can declutter noisy rows
// (e.g. high-volume embedding models) without losing them from history. Stored
// in localStorage like the column-visibility toggles.
function loadHiddenFilter(key: string): Set<string> {
...
function persistHiddenFilter(key: string, values: Set<string>): void {
  try {
...
export interface RequestLoggerV2Props {
  initialSelectedId?: string;
  compact?: boolean;
}

const RequestLoggerV2 = forwardRef<RequestLoggerV2Handle, RequestLoggerV2Props>((props, ref) => {
  const { initialSelectedId, compact = false } = props;
  ...
  const [hiddenModels, setHiddenModels] = useState<Set<string>>(() =>
    loadHiddenFilter("loggerHiddenModels")
  );
  const [hiddenProviders, setHiddenProviders] = useState<Set<string>>(() =>
    loadHiddenFilter("loggerHiddenProviders")
  );
  ...
  const filteredLogs = useMemo(() => {
    ...
    // Persisted hide filters (declutter noisy models/providers)
    if (hiddenModels.size > 0) {
      arr = arr.filter((l) => !hiddenModels.has(l.model) && !hiddenModels.has(l.requestedModel));
    }
    if (hiddenProviders.size > 0) {
      arr = arr.filter((l) => !hiddenProviders.has(l.provider));
    }

    return arr;
  }, [logs, activeFilter, hiddenModels, hiddenProviders]);
  ...
  return (
    <div className={`flex flex-col ${compact ? "gap-2 h-full" : "gap-4"}`}>
      {compact ? (
        <div className="flex items-center justify-between gap-2 px-1">
          <div className="relative flex-1 max-w-sm">
            <input
              ...
              className="w-full pl-8 pr-7 py-1 text-xs rounded-md bg-bg-subtle border border-border text-text-primary placeholder:text-text-muted focus:outline-none focus:border-primary"
            />
            <span className="material-symbols-outlined absolute left-2 top-1/2 -translate-y-1/2 text-text-muted text-[15px] pointer-events-none">
              search
            </span>
            {search && (
              <button
          ...
          <div className="flex items-center gap-2 text-[11px] text-text-muted">
            <span className="font-mono font-semibold text-text-primary">{sortedLogs.length}</span>{" "}
            {t("shown")}
          </div>
        </div>
      ) : (
        <>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="flex-1 min-w-[160px] relative">
              ...
              <input
                ...
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                className="w-full pl-8 pr-2.5 py-1 rounded-lg bg-bg-subtle border border-border text-xs text-text-primary placeholder:text-text-muted focus:outline-none focus:border-primary"
              />
            </div>
            ...
            <div className="min-w-[150px] relative">
              ...
              <input
                ...
                className="w-full pl-7 pr-2.5 py-1 rounded-lg bg-bg-subtle border border-border text-xs text-text-primary font-mono placeholder:text-text-muted focus:outline-none focus:border-primary"
              />
            </div>
            ...
            <select
              ...
              className="px-2.5 py-1 rounded-lg bg-bg-subtle border border-border text-xs text-text-primary focus:outline-none focus:border-primary appearance-none cursor-pointer min-w-[140px]"
            >
            ...
            <div className="relative">
              <select
                ...
                className="px-2.5 py-1 rounded-lg bg-bg-subtle border border-border text-xs text-text-primary focus:outline-none focus:border-primary appearance-none cursor-pointer min-w-[180px]"
              >
              ...
              {selectedModel && (
                <button
                  onClick={() =>
                    toggleHiddenFilter("loggerHiddenModels", selectedModel, setHiddenModels)
                  }
                  title={t("hideModel")}
                  className="absolute right-1.5 top-1/2 -translate-y-1/2 p-0.5 rounded hover:bg-bg-subtle text-text-muted hover:text-red-500 transition-colors"
                >
            ...
            <select
              ...
              className="px-2.5 py-1 rounded-lg bg-bg-subtle border border-border text-xs text-text-primary focus:outline-none focus:border-primary appearance-none cursor-pointer min-w-[160px]"
            >
            ...
            <div className="flex items-center gap-2 text-xs text-text-muted">
              <span className="px-1.5 py-0.5 rounded bg-bg-subtle border border-border font-mono">
                {totalCount} {t("total")}
              </span>
              {runningCount > 0 && (
                <span className="px-1.5 py-0.5 rounded bg-amber-500/10 text-amber-700 dark:text-amber-400 font-mono">
                  {runningCount} {t("running")}
                </span>
              )}
              <span className="px-1.5 py-0.5 rounded bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 font-mono">
                {okCount} {t("ok")}
              </span>
              {errorCount > 0 && (
              ...
              {comboCount > 0 && (
                <span className="px-1.5 py-0.5 rounded bg-violet-500/10 text-violet-700 dark:text-violet-400 font-mono">
                  {comboCount} {t("combo")}
                </span>
              )}
              {apiKeyCount > 0 && (
            ...
            <select
              ...
              className="px-2.5 py-1 rounded-lg bg-bg-subtle border border-border text-xs text-text-primary focus:outline-none focus:border-primary appearance-none cursor-pointer min-w-[150px]"
              title={t("sortLogs")}
            >
            ...
            <div className="flex items-center gap-1">
              ...
              <input
                ...
                className="w-10 text-center text-[10px] bg-transparent border border-border rounded px-1 py-0.5 text-text-primary [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {uniqueProviders.map((p) => {
              ...
              const isHidden = hiddenProviders.has(p);
              return (
                <div
                  key={p}
                  className={`flex items-center gap-0.5 rounded-full border transition-all ${
                    isHidden ? "opacity-40" : ""
                  } ${isActive ? "border-white/40 ring-1 ring-white/20" : "border-transparent"}`}
                  style={{
                    backgroundColor: isHidden ? "#111827" : isActive ? pc.bg : `${pc.bg}33`,
                  }}
                >
                  <button
                    ...
                    style={{ color: isActive || isHidden ? "#fff" : pc.bg }}
                  >
                    {displayLabel}
                  </button>
                  <button
                    onClick={() =>
                      toggleHiddenFilter("loggerHiddenProviders", p, setHiddenProviders)
                    }
                    title={isHidden ? t("showProvider") : t("hideProvider")}
                    className="px-1.5 py-1 rounded-full text-xs hover:bg-black/10 transition-colors"
                    style={{ color: isActive || isHidden ? "#fff" : pc.bg }}
                  >
          ...
          {/* Hidden Items Manager (declutter noisy models/providers) */}
          {(hiddenModels.size > 0 || hiddenProviders.size > 0) && (
            <div className="flex flex-wrap items-center gap-1.5 bg-bg-subtle/60 border border-border rounded-lg px-2 py-1.5">
              <span className="text-[10px] text-text-muted uppercase tracking-wider mr-1">
                {t("hiddenItems")}
              </span>
              {[...hiddenProviders].map((p) => (
                <button
                  key={`p-${p}`}
                  onClick={() => toggleHiddenFilter("loggerHiddenProviders", p, setHiddenProviders)}
                  title={t("showProvider")}
                  className="flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-medium bg-red-500/15 text-red-700 dark:text-red-300 border border-red-500/30 hover:bg-red-500/25 transition-colors"
                >
                  <span className="material-symbols-outlined text-[12px]">visibility</span>
                  {getProviderDisplayLabel(p, providerNodes) || p.toUpperCase()}
                </button>
              ))}
              {[...hiddenModels].map((m) => (
                <button
                  key={`m-${m}`}
                  onClick={() => toggleHiddenFilter("loggerHiddenModels", m, setHiddenModels)}
                  title={t("showModel")}
                  className="flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-medium bg-red-500/15 text-red-700 dark:text-red-300 border border-red-500/30 hover:bg-red-500/25 transition-colors"
                >
                  <span className="material-symbols-outlined text-[12px]">visibility</span>
      ...
      <Card
        ...
        className={
          compact
            ? "h-full max-h-full overflow-hidden bg-surface flex flex-col border border-border"
            : "min-h-[460px] resize-y overflow-auto bg-surface"
        }
      >
        <div
          ...
          className={
            compact
              ? "p-0 overflow-x-auto overflow-y-auto flex-1 h-full"
              : "p-0 overflow-x-auto overflow-y-auto h-full min-h-[460px]"
          }
        >
