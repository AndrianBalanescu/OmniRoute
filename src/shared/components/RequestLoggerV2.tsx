...
const RequestLoggerV2 = forwardRef<
  RequestLoggerV2Handle,
  { initialSelectedId?: string; compact?: boolean }
>((props, ref) => {
  const { initialSelectedId, compact } = props as any;
  ...
  return (
    <div className={`flex flex-col ${compact ? "gap-1" : "gap-4"}`}>
      {!compact && (
        <>
          {/* Header Bar */}
      ...
      <Card
        padding="none"
        className={`resize-y overflow-auto bg-surface ${compact ? "min-h-0" : "min-h-[460px]"}`}
      >
        <div
          ref={scrollContainerRef}
          className={`p-0 overflow-x-auto overflow-y-auto h-full ${
            compact ? "min-h-0" : "min-h-[460px]"
          }`}
        >
